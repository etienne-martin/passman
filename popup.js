...
chrome.tabs.query({
	...
	if( domain ){
		var hostname = domain.toLowerCase().replace(/^(www\.)/,"").split(".").reverse();
			hostname = hostname[1]+"."+hostname[0]; // Remove sub domains
	}else{
		hostname = "No domain found";	
	}
	
	domainLabel.value = hostname;
	
	chrome.tabs.executeScript(tabs[0].id, { file: 'tab.js' }, function() {
        ...
        mainInput.addEventListener('input', function(e){
			...
			chrome.tabs.sendMessage(tabs[0].id, { type: 'sendPassword', hash: password });
...
var domainLabel = document.getElementsByTagName("input")[1];
